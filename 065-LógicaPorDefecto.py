...
class Regla:
    def __init__(self, antecedentes, consecuencia, por_defecto=False):
    ...
    def evaluar(self, hechos):
        for antecedente in self.antecedentes:
            if antecedente not in hechos:
                return False
        return True or self.por_defecto
...
# Definir reglas
regla1 = Regla(['pajaro_vuela', 'pajaro'], 'animal_vuela', por_defecto=True)
regla2 = Regla(['pez_nada', 'pez'], 'animal_nada', por_defecto=True)
regla3 = Regla(['gato_maulla', 'gato'], 'animal_mamifero', por_defecto=True)
regla4 = Regla(['tiburon_come'], 'tiburon', por_defecto=True)
